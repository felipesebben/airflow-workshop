...
from sqlalchemy import MetaData, Table, and_, create_engine, delete
...
def transfer_and_delete_data():
    hook = PostgresHook(postrgres_conn_id="postgres_render")
    engine = hook.get_sqlalchemy_engine()
...
with DAG(
    ...
    schedule_interval="*/5 * * * *",  # Executar a cada cinco minutos.
    catchup=False,
) as dag:
    ...
    delete_null_titles_task = PythonOperator(
        task_id="delete_null_titles_task",
        python_callable=delete_null_titles,
    )
    delete_negative_values_task = PythonOperator(
        task_id="delete_negative_values_task",
        python_callable=delete_negative_values,
    )
